...
icon = Image.open("D:/Dimpu/ProjectWorks/Python/bizcardx/bizcard.jpeg")
...
# TABLE CREATION
mycursor.execute('''CREATE TABLE IF NOT EXISTS bizcarddata
...
if selected == "Home":
    ...
    with col1:
        st.image(Image.open("D:/Dimpu/ProjectWorks/Python/bizcardx/bizcard.jpeg"),width=500)
...
if selected == "Upload & Extract":
    if st.button(":blue[Already stored data]"):
        mycursor.execute(
            "select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from bizcarddata")
    ...
    if uploaded_card is not None:
        ...
        if st.button("Upload to Database"):
            for i, row in df.iterrows():
                ...
                sql = """INSERT INTO bizcarddata(company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code,image)
        ...
        if st.button(":blue[View updated data]"):
            mycursor.execute("select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from bizcarddata")
            updated_df = pd.DataFrame(mycursor.fetchall(),
                                          columns=["Company_Name", "Card_Holder", "Designation", "Mobile_Number",
                                                   "Email",
                                                   "Website", "Area", "City", "State", "Pin_Code"])
...
if selected == "Modify":
    ...
    if select == "ALTER":
        ...
        try:
            mycursor.execute("SELECT card_holder FROM bizcarddata")
            ...
            if selected_card == "None":
                ...
                mycursor.execute(
                "select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from bizcarddata WHERE card_holder=%s",
                (selected_card,))
                ...
                if st.button(":blue[Commit changes to DB]"):
                   ...
                   # Update the information for the selected business card in the database
                    mycursor.execute("""UPDATE bizcarddata SET company_name=%s,card_holder=%s,designation=%s,mobile_number=%s,email=%s,website=%s,area=%s,city=%s,state=%s,pin_code=%s
        ...
        try:
            ...
            if selected_card == "None":
                ...
                if st.button("Yes Delete Business Card"):
                    mycursor.execute(f"DELETE FROM bizcarddata WHERE card_holder='{selected_card}'")
                    mydb.commit()
